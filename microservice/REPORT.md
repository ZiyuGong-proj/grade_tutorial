...
4. Included the grade dataset in `data/Grade Table.csv` so containers have local access to the input table.
...
- **Data sharing**: Ensuring each container had access to the grade table was solved by copying `data/Grade Table.csv` into every image build context.
